...
int main(int argc, char **argv) {
    ...
    struct sexp *total = parse_line("(1 1.2)");
    ...
    struct sexp *a = pop_front_sexp_array(&total->list);
    struct sexp *b = pop_front_sexp_array(&total->list);
    struct sexp *c = pop_front_sexp_array(&total->list);

    printf("%s\n", is_int_sexp(a) ? "YES" : "NOPE");
    printf("%s\n", is_float_sexp(b) ? "YES" : "NOPE");
    ...
    //assert(is_empty_list_sexp(s6) == FALSE);
